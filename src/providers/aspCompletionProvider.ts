    ...
    private provideKeywordCompletions(): vscode.CompletionItem[] {
        return VBSCRIPT_KEYWORDS.map(kw => {
            ...
            if (kw.keyword === 'If') {
                ...
                item.insertText = new vscode.SnippetString('For $1 = $2 To $3\n\t$0\nNext');
            } else if (kw.keyword === 'For Each') {
                item.insertText = new vscode.SnippetString('For Each $1 In $2\n\t$0\nNext');
            } else if (kw.keyword === 'While') {
                item.insertText = new vscode.SnippetString('While $1\n\t$0\nWend');
            } else if (kw.keyword === 'Do') {
                item.insertText = new vscode.SnippetString('Do\n\t$0\nLoop');
            } else if (kw.keyword === 'Select Case') {
                item.insertText = new vscode.SnippetString('Select Case $1\n\tCase $2\n\t\t$0\nEnd Select');
            } else if (kw.keyword === 'Sub') {
                item.insertText = new vscode.SnippetString('Sub $1($2)\n\t$0\nEnd Sub');
            } else if (kw.keyword === 'Function') {
                item.insertText = new vscode.SnippetString('Function $1($2)\n\t$0\nEnd Function');
            }
